...
def PlotLikertOverConditions(tb,nPoint,customLikertRange=None,tb2=None,customLikertRange2=None):
    # This functions gets a table of questions and their responces in likert scale (1:positive N:negative) as coulmns, as well as a another column indicating the condition of the response
    ...
    font1 = {'family': 'sans-serif','color':  'black','weight': 'normal','size': 16,}
    font2 = {'family': 'sans-serif','color':  'grey','weight': 'normal','size': 18,}
    ...
    for cond in df['condition'].unique():
        ...
        for q in range(1,len(Qs)):
            for i in range(1,nPoint+1):
                q_s = Qs[q]
                temp2.loc[q_s, i]=g(temp.loc[q_s,:].value_counts(),i)
...
#                                 p.set_hatch('/ '*cond)
                            patch_handles[j].text(
                                ...
                                p.get_y() + 0.0195 + p.get_height() /(len(Qs)-1),
...
#                 'C ' + str(cond+1),
                ...
                ha="center",
                fontdict=font2,
                bbox={'edgecolor':'none','facecolor':'white', 'alpha':1.0, 'pad':2})
...
#     print yticks
    ...
    for i,y_tick in enumerate(yticks):
        ...
        y=yticks[i]-0.4
        ax.text(x,y, v[0],fontsize = 12,zorder = 6, color = 'white',horizontalalignment='right',
                    bbox={'edgecolor':'none','facecolor':likert_colors[1], 'alpha':1.0, 'pad':4})  
        
        middle_colors=likert_colors[1:-1]
        for ci,c in enumerate(middle_colors):
            x=x+0.15
            ax.text(x,y,' ',fontsize = 12,zorder = 6, color = 'white',horizontalalignment='right',
                    bbox={'edgecolor':'none','facecolor':middle_colors[ci], 'alpha':1.0, 'pad':4})  
            
            
        ax.text(x+0.1,y,v[1],fontsize = 12,zorder = 6, color = 'white',horizontalalignment='left',
                    bbox={'edgecolor':'none','facecolor':likert_colors[-1], 'alpha':1.0, 'pad':4})  
    plt.tight_layout()
